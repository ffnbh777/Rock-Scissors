...
function playRound(){
  ...
  if(
    ...
    results.player += 1
    round.push(`You win ${playerChoice} beats ${computerChoice}`)
  } else if ( playerChoice === computerChoice) {
    results.ties += 1
    round.push(`You tie  You both chose ${playerChoice}`)
  } else {
    results.computer += 1
    round.push(`You lose  ${computerChoice} beats ${playerChoice}`)
  }
...
function displayContent(results, round){
  let winner;
  if(results.player >=1 || results.computer >=1) {
    if(results.player >=1) {
      winner = ''
